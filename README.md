...
这个版本按本地应用运行，不再提供直接双击 `index.html` 或 GitHub Pages 静态模式。向量检索和 SQLite 历史数据库都需要随项目提供的 Node.js 本地服务。
...
本地完整模式使用 Node.js 内置的 `node:sqlite`，无需额外安装数据库：

```text
server/data/contract-reviewer.sqlite
```

数据库可能包含合同正文、审核问题、聊天内容、履约台账和法律依据结果，已被 `.gitignore` 排除。请勿手动提交或分享该文件及其 `-wal`、`-shm` 文件。

旧版浏览器历史会在首次连接本地服务时迁移到 SQLite。加载已有记录时直接恢复当时保存的法律依据，不会重新调用 API；需要重新判断时可手动点击“重新核对”。
